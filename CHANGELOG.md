...
- **Daily-rolling daemon log file with retention (123b8171)** -- `tracing-appender` daily rolling replaces the never-rolled `conflabd.log` appender. New `[log] rotation = "daily"|"never"` and `[log] max_files = N` (default 14) in `daemon.toml`; older dated files (`conflabd.YYYY-MM-DD.log`) are deleted automatically. New `crate::log_files` Highlander module owns log-file naming; readers (`daemon_logs` MCP tool, `/logs` resource endpoint, CLI `conflab daemon logs`) all consume `resolve_log_path`. `rotation = "never"` preserves the legacy single-file behaviour. Startup session-separator retired in favour of a structured `info!` line including version + git hash.
- **`clearLogs` GraphQL mutation (6acc955c)** -- `mutation { clearLogs { filesDeleted bytesFreed } }` deletes every dated log file strictly before today (UTC). Today's actively-written file and the legacy un-dated `conflabd.log` are preserved. Returns counts so the UI can confirm concretely. New `time_util::today_utc_iso()` Highlander.
- **Clear Logs button in macOS Daemon panel (dbdb1245)** -- new **Logs:** row alongside Diagnostics with a **Clear Logs...** button on the Daemon tab. Two-step confirmation (warn + final commit-style check). Calls the daemon's `clearLogs` mutation; the view is a thin coordinator. Result alert shows deleted-file count and bytes-freed.

### Fixed

- **Shape silently dropped from lens body composition (919e7e69)** -- `execute_prompt_template` had an if/else that only handled the body-empty path; body-non-empty fell through to body-as-is, ignoring `shape:` completely. Replaced with an exhaustive 4-arm match over `(body-empty, shape-present)`. Two regression tests in `mgmt/tests.rs` cover both shape-only and body+shape paths.
- **macOS FSEvents periodic resync backstop (919e7e69, 90ffe918)** -- `fs_watcher` had no resync backstop, so dropped/coalesced FSEvents for new subdirs left the SQLite index out of sync until daemon restart. New `WatcherConfig.resync_interval_secs` (default 60s, 0 disables) drives a periodic `FullResync`. Tests use `tokio::time::pause` + `advance` for deterministic timer assertions; `tokio` test-util added as dev-dep. Companion commit 90ffe918 suppresses the chatty no-op resync logs (entry log dropped, completion log gated on a positive change count). Tracked at `intent/issues/OPEN/0001/0001-fsevents-resync-fragility.md`; permanent kqueue fix deferred.
- **Highlander on web Lens runner's file-upload variable types (f3c9fb61)** -- both the upload-pipeline configuration and the field-to-upload lookup filtered on the literal `"text+files"` string, missing `texteditor+files` variables, so the (+) attach icon never appeared for those vars. Single source of truth: `@file_var_types` module attribute in `LensesLive.Helpers`. `configure_uploads/2` and `upload_for_field/2` both read from it; `file_input_variables/1` extracted as a pure helper.

### Changed

- **llm_db 2026.4.7 -> 2026.4.8.** Routine dependency refresh.
...
- **ST0106 -- Schema-enforced shapes via Anthropic tool-use (426e291d, 6af88820).** Moves shapes into tool-use `input_schema` with `tool_choice` forcing the call so the model returns JSON the daemon validates and renders against the shape template. Refront commit re-roots the design in lens/shape orthogonality and adds the 2026-04-29 audit of the four shape-bearing personal lenses. Sized L (8 WPs).
- **ST0107 -- Named-tool registry for symbolic tool references (426e291d).** New `~/.conflab/db/tools/<slug>.tool.json` file type joins lenses and shapes; lenses, daemon config, and agent reasoning loops reference tools by symbolic slug. Compile-time slug constants + startup validation make typos fail fast. Sized L (12 WPs); lands before ST0106.
- **Issue 0001 -- FSEvents resync fragility (c345fe96).** Captures the macOS-FSEvents-drops-events-for-new-subdirs root cause, the periodic-resync mitigation shipped this release, and permanent-fix options for when watcher hardening is scheduled.

## [0.3.4] - 2026-04-28

Patch release on top of v0.3.3. Headline is **UI/UX Polish round (ST0099 WP-03..WP-09)** -- a themed seven-WP pass across the macOS Launcher, the Phoenix web Lens runner, the Flabs chat surface, and the Manage window. Notable user-facing changes: the **web Lens runner** now uses CodeMirror for `text` variables (was textareas; this also fixes a latent bug where text-editor variable values were silently lost on submit); **Lens tiles** wrap their titles instead of clipping; **Flabs chat** gains a textarea composer with Enter-send / Shift-Enter newlines / IME guard / auto-grow / caret-line-aware history navigation, plus a new **Clear flab messages** action under the kebab menu; the **Swift Launcher Run page** gets a `Running... / Done · 1.2s` status pill in the header and chevron-only disclosure (less chrome); a **⌥ (Option) key reveal** idiom now exposes tile shape ids, Run page shape names, the cog menu's Model picker and Rendered toggle, and the build stamp; the menubar gains a **Help** item; and twelve inline two-step destructive-confirm rows across the codebase now share a single canonical `<.confirm_action>` component with consistent labels (`Delete?` / `Delete` / `Cancel`) and `btn-error` styling.

### Added

- **Web Lens runner: CodeMirror for `text` variables (ST0099/WP-03)** -- the Phoenix web Lens runner now uses CodeMirror via the existing `NotebookCellHook` for `text` and `text+files` variable inputs. Brings the web side to parity with the Swift Launcher Run page. Latent value-loss bug fixed as a side effect: `texteditor` and `texteditor+files` Lens-runner variables previously had their values silently dropped on submit because the hooked div had no `name=` attribute and the form's `update-var` phx-change handler only saw form-serialised data. New hidden-input mirror pattern puts a sibling `<input type="hidden">` next to the editor; the hook updates `.value` AND dispatches a bubbling `input` event so phx-change fires.
- **Lens tile title wrap (ST0099/WP-04)** -- Lens tiles in the Swift Launcher and across the three Phoenix web tile sites (`dir_tile`, `lens_tile`, `theme_tile`) wrap their titles instead of clipping at two lines.
- **Flabs chat overhaul + Clear flab messages (ST0099/WP-05)** -- new `Conflab.Collaboration.clear_flab_messages/2` domain action empties a flab's message stream while keeping the flab, participants, and tasks intact (creator + owner/admin only; archived flabs blocked). UI: kebab menu gains **Clear** between Details and Delete with a two-step confirm row matching its Delete sibling. Composer flips from single-line `<input>` to a `<textarea>` with Enter-send / Shift-Enter newlines / IME guard / auto-grow up to 6 lines / caret-line-aware history navigation. Bubble density: `chat-bubble-sm` with inline `·`-separated timestamp. Header gains participant count with singular/plural agreement.
- **Swift Launcher Run page chrome trim (ST0099/WP-06)** -- new `RunStatusPill` in the header: `idle` hidden, `running` shows `Running...`, `complete` shows `Done · 1.2s`, error shows `Error · <reason>`, tokens count appears under ⌥. The Lens-details disclosure drops its `Hide / Show Lens details` text label -- chevron alone in a 22x22 hit target with a `.help(...)` tooltip.
- **⌥ (Option) key reveal across the Launcher (ST0099/WP-07)** -- holding ⌥ Option on the macOS Launcher reveals power-user details: tile shape ids, Run page shape name in the header, cog menu's Model picker and Rendered toggle, build stamp, and `RunStatusPill` tokens count. New canonical `OptionKeyMonitor.shared` Highlander wraps a single `NSEvent.addLocalMonitorForEvents(matching: .flagsChanged)` listener for the entire app -- adding a private ⌥ listener anywhere is an explicit regression.
- **Help menu item in macOS menubar (ST0099/WP-08)** -- new **Conflab Help** item between `Open Conflab` and `Quit`. Opens `<server>/app/help` in the user's default browser. Server URL resolves via new `ConflabConfig.resolvedServerBaseURL(fallback:)` Highlander.
- **Canonical `<.confirm_action>` component (ST0099/WP-09)** -- new `ConflabWeb.Components.ConfirmAction.confirm_action/1` replaces twelve drifting inline two-step destructive-confirm rows across the Phoenix web layer. Standard label vocabulary now enforced: prompt = `{verb}?`, confirm button = `{verb}` (`btn-error`), cancel button = `Cancel` (`btn-ghost`).

### Changed

- **Oban 2.22.0 + migration v14.** `mix.lock` bumps Oban from 2.21.1 to 2.22.0. After upgrading, run `mix ash.migrate` and `MIX_ENV=test mix ash.migrate` -- the app refuses to boot with `Oban migrations are outdated` until v14 is applied.
